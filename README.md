...
0. 처음 구글로 로그인을 하면 닉네임을 정할수 있도록 만들기
1. 메뉴 생성 (기존에 구글 로그인 버튼이 위치한 자리에 메뉴 토글을 만들고 그거 누르면 메뉴가 뜨도록 -> 비로그인시에는 구글 로그인 버튼만 뜨고, 로그인했을때는 마이페이지와 같은 옵션 넣기)
2. 마이페이지에 들어갈 내용: 닉네임, 프로필사진 (기본적으로 정해진 프사는 있지만 사용자가 알아서 바꿀수 있도록), 현재 보유하고 있는 주식종류/개수/각 종목별 구매한 금액(샀을때에 비해 현재 얼마나 오르고 내렸는지 실시간으로 바뀌도록)

### 1순위: 주문·체결의 정확성

- [ ] `trades` 체결 테이블을 추가해 하나의 주문에서 발생한 개별 체결을 별도로 저장
- [ ] 시장가 주문도 현재가 즉시 체결이 아니라 매도·매수 호가를 순서대로 소진하며 체결
- [ ] 지정가 매수 주문의 현금과 지정가 매도 주문의 주식을 주문 시점에 예약
- [ ] 예약된 금액·수량을 체결·취소·만료 시 정확히 반환
- [ ] 부분 체결마다 체결 수량, 체결 가격, 체결 시각을 기록
- [ ] 주문 취소 API와 화면 추가
- [ ] 가격·시간 우선 매칭에서 실제 체결을 유발한 주문과 대기 주문을 구분하고 체결 가격 규칙을 명확히 적용

### 2순위: 가격 형성 및 시장 규칙

- [ ] 봇이 항상 매수·매도 주문을 교차시키지 않도록 현실적인 스프레드와 유동성 모델 적용
- [ ] 사용자 주문량, 체결량, 호가 잔량을 가격 변동에 반영
- [ ] 종목별 호가 단위(tick size)와 주문 가능 가격 검증
- [ ] 거래 시간, 장 시작·마감, 장외 시간 주문 처리 추가
- [ ] 일일 가격 제한폭, 거래 중단, 급등락 보호 규칙 추가
- [ ] 시장가 주문에 호가 부족·슬리피지 상황을 반영
