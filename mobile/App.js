...
export default function App() {
  ...
  return <SafeAreaView style={styles.safe}><NewsModal news={selectedNews} stocks={market.stocks} onClose={() => setSelectedNews(null)} styles={styles} t={t} /><ScrollView contentContainerStyle={styles.page}>
    <View style={styles.header}><View><Text style={styles.brand}>GameStock</Text><Text style={styles.status}>{t(status)}</Text></View><TouchableOpacity style={styles.loginButton} onPress={() => setMenuOpen(previous => !previous)}><Text style={styles.loginText}>{user ? `${account?.nickname || t('내 계정')} ▾` : `☰ ${t('메뉴')}`}</Text></TouchableOpacity></View>
...
const createStyles = dark => {
  ...
  const overrides = {
    ...
    menuItem:{backgroundColor:'#1d1924'}, title:{color:'#f0edf7'}, card:{backgroundColor:'#1d1924'}, code:{color:'#aaa4b8'}, name:{color:'#f0edf7'},
    ...
    event:{backgroundColor:'#1d1924'}, eventTitle:{color:'#f0edf7'}, openOrderRow:{borderTopColor:'#3b3648'}, cancelOrder:{borderColor:'#514960',backgroundColor:'#26212f'},
