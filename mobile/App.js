...
const describePriceReason = event => event?.priceReason || (Number(event?.priceChangePercent || 0) > 0 ? '최근 거래 흐름을 따라 가격이 상승하고 있습니다.' : Number(event?.priceChangePercent || 0) < 0 ? '최근 거래 흐름을 따라 가격이 하락하고 있습니다.' : '최근 가격은 보합 상태입니다.');
...
function NewsModal({ news, stocks, onClose }) {
...
export default function App() {
  ...
  const priceReason = event => describePriceReason(event);
  ...
  return <SafeAreaView style={styles.safe}><NewsModal news={selectedNews} stocks={market.stocks} onClose={() => setSelectedNews(null)} /><ScrollView contentContainerStyle={styles.page}>
    <View style={styles.header}><View><Text style={styles.brand}>GameStock</Text><Text style={styles.status}>{status}</Text></View><TouchableOpacity style={styles.loginButton} onPress={() => setMenuOpen(previous => !previous)}><Text style={styles.loginText}>{user ? `${account?.nickname || '내 계정'} ▾` : '☰ 메뉴'}</Text></TouchableOpacity></View>
    {menuOpen && <View style={styles.menuPanel}>{!user && <TouchableOpacity style={styles.menuItem} onPress={() => { setMenuOpen(false); promptAsync(); }} disabled={!request}><Text style={styles.menuLoginText}>Google 로그인</Text></TouchableOpacity>}{user && <TouchableOpacity style={styles.menuItem} onPress={openProfile}><Text>마이페이지</Text></TouchableOpacity>}<TouchableOpacity style={styles.menuItem} onPress={openRanking}><Text>랭킹</Text></TouchableOpacity><Text style={styles.riverMenu}>{riverTemperature?.available && riverTemperature.temperature != null ? `한강 수온(선유) ${Number(riverTemperature.temperature).toFixed(1)}℃` : '한강 수온(선유) 조회 불가'}</Text>{user && <TouchableOpacity style={styles.menuItem} onPress={() => { setMenuOpen(false); signOut(firebaseAuth); }}><Text>로그아웃</Text></TouchableOpacity>}</View>}
...
styles.eyebrow = { color: '#6750a4', fontSize: 11, letterSpacing: 1.2, fontWeight: '800', marginBottom: 6 };
styles.eventContent = { flex: 1, marginRight: 10 };
styles.eventMeta = { color: '#697386', fontSize: 11, marginTop: 4 };
styles.eventReason = { color: '#697386', fontSize: 11, lineHeight: 17, marginTop: 5 };
styles.eventNeutral = { color: '#697386', fontWeight: '700' };
styles.newsContent = { flex: 1, marginRight: 8 };
styles.newsModalReason = { marginTop: 14, padding: 11, borderRadius: 9, backgroundColor: '#f7f5fc', color: '#6750a4', fontWeight: '700', lineHeight: 19 };
styles.newsModalSummary = { marginTop: 10, color: '#17202f', lineHeight: 21 };
styles.newsModalPriceRow = { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginTop: 4 };
styles.newsModalSource = { marginTop: 12, alignSelf: 'flex-start' };
styles.newsModalSourceText = { color: '#6750a4', fontSize: 12, fontWeight: '800' };
styles.priceDrivers = { marginTop: 4, paddingBottom: 4 };
styles.priceDriversUpdated = { marginTop: 3, color: '#697386', fontSize: 10, lineHeight: 15 };
styles.driverGrid = { flexDirection: 'row', flexWrap: 'wrap', gap: 7, marginTop: 8 };
styles.driverCard = { width: '48%', padding: 9, borderWidth: 1, borderColor: '#eeeaf5', borderRadius: 9, backgroundColor: '#faf9fc' };
styles.driverLabel = { color: '#697386', fontSize: 11, fontWeight: '700' };
styles.driverValue = { marginTop: 4, fontSize: 11, lineHeight: 16 };
