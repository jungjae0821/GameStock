...
export default function App() {
  ...
  const openProfile = async () => { setMenuOpen(false); setProfileOpen(true); setProfileMessage('불러오는 중입니다…'); try { const [nextProfile, nextOrders] = await Promise.all([api('/api/profile'), api('/api/orders')]); setProfile(nextProfile); setOpenOrders(nextOrders); setNickname(nextProfile.nickname || ''); setProfileMessage(''); } catch (error) { setProfileMessage(error.message); } };
