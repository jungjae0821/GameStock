...
2. 웹은 루트에서 `npm run dev` 또는 `scripts/run-frontend.ps1`을 실행한다. 웹 API 주소는 Spring Boot의 `8081`로 연결한다.
...
5. 웹 배포 시 `VITE_API_BASE_URL`을 공개 백엔드 주소로 지정한다. 로컬에서는 `.env.local`의 `VITE_API_BASE_URL=http://localhost:8081`을 사용한다.
